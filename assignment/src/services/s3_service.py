...
class S3Service:
    ...
    def get_presigned_upload_url(self, image_id, metadata):
        return self.s3_client.generate_presigned_url('put_object', Params={'Bucket': self.bucket_name, 'Key': image_id,
                                                                           'Metadata': metadata}, ExpiresIn=3600)

    def get_presigned_download_url(self, image_id):
        return self.s3_client.generate_presigned_url('get_object', Params={'Bucket': self.bucket_name, 'Key': image_id},
                                                     ExpiresIn=3600)
